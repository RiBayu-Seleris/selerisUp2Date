...
onMounted(() => {
  ...
  const thresholdValue = 0.4; // misal ingin 40% terlihat

  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        // ✅ Tambahkan if untuk threshold
    ...
    {
      root: null,
      threshold: [0, thresholdValue, 1], // bisa pakai array untuk deteksi beberapa titik
    }
  );
  ...
  <div ref="lineRef" class="w-auto h-auto flex items-center">
    <div class="relative flex flex-row w-auto h-full">
      ...
      >
        <div
          :class="[
            `w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 p-[5px] sm:p-[6px] md:p-[6px] xl:p-[8px] rounded-full ${circleColor} cursor-pointer`,
          ]"
        >
          <div
